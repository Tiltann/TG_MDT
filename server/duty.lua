...
local DutyCache     = {}
local LastJobCache  = {}
local dutyConfigCache = nil

---@return table
local function getDutyConfig()
    if dutyConfigCache then return dutyConfigCache end
...
local function hasAccess(src)
    ...
    if not allowedJobsLookup then
        local allowed = (Config.MDT and Config.MDT.allowed_jobs) or {}
        if #allowed == 0 then
            allowedJobsLookup = 'all'
        else
            allowedJobsLookup = {}
            for i = 1, #allowed do
                allowedJobsLookup[string.lower(allowed[i])] = true
            end
        end
    end

    if allowedJobsLookup == 'all' then return true end
    return allowedJobsLookup[string.lower(job)] == true
end
