...
local function warnAkteDefault(kind, key, message)
    Debug.warn(('Akte default resolver failed (%s.%s): %s'):format(kind, key, message))
end
...
RegisterNetEvent('TG_MDT:server:joinRadioChannel', function(freq)
    ...
    if #freqStr > 32 then
        Debug.warn(('joinRadioChannel: frequency too long from player %s'):format(src))
        return
    end
