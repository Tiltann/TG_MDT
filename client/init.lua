...
local function loadLocaleDictionary(locale)
	...
	local key_count = 0
	for _ in pairs(decoded) do key_count = key_count + 1 end
	Debug.debug(('Loaded locale dictionary: %s | keys=%s'):format(path, tostring(key_count)))
	return decoded
end
...
RegisterNetEvent('TG_MDT:dutyStateChanged', function(payload)
	...
	local ok, player_data = pcall(buildPlayerUiData)
	NUI.send('setData', {
		key = 'player',
		value = ok and player_data or {},
	})
end)
