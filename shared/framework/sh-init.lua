...
local function detect()
    ...
    if esx == 'started' or esx == 'starting' then
        if detectedName ~= 'esx' then
            Debug.debug('Framework detect: es_extended has been detected as ' .. esx)
            detectedName = 'esx'
    ...
    if qbox == 'started' or qbox == 'starting' then
        if detectedName ~= 'qbox' then
            Debug.debug('Framework detect: qbx_core has been detected as ' .. qbox)
            detectedName = 'qbox'
    ...
    if qb == 'started' or qb == 'starting' then
        if detectedName ~= 'qbcore' then
            Debug.debug('Framework detect: qb-core has been detected as ' .. qb)
            detectedName = 'qbcore'
...
setmetatable(Framework, {
    __index = function(t, key)
        if key == 'name' then
            local name = detect()
            Debug.debug(('Framework name index read: resolved to "%s"'):format(name))
            return name
        end
        if key == 'Server' then
            local name = detect()
            Debug.debug(('Framework Server index read: resolved and routing to "%s" bridge Server'):format(name))
            return Framework.Bridges[name].Server
        end
        if key == 'Client' then
            local name = detect()
            Debug.debug(('Framework Client index read: resolved and routing to "%s" bridge Client'):format(name))
