...
const EMPTY_DRAFT: DraftIncident = {
  title: "",
  description: "",
  location: "",
  severity: "medium",
...
const severityTone: Record<IncidentRecord["severity"], string> = {
...
const statusTone: Record<IncidentRecord["status"], string> = {
  open: "text-sky-300 bg-sky-500/10",
...
export default function IncidentsView({
  ...
  onCreate: (incident: Omit<IncidentRecord, "id" | "createdAt" | "updatedAt">) => void;
  onUpdate: (id: string, patch: Partial<IncidentRecord>) => void;
  onDelete: (id: string) => void;
}) {
  const [open, setOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [draft, setDraft] = useState<DraftIncident>(EMPTY_DRAFT);
  const [personSearch, setPersonSearch] = useState("");
  const [vehicleSearch, setVehicleSearch] = useState("");

  const ordered = useMemo(() => incidents.slice().sort((a, b) => Date.parse(b.updatedAt) - Date.parse(a.updatedAt)), [incidents]);
  ...
  const openEdit = (incident: IncidentRecord) => {
    ...
    setDraft({
      ...
      linkedPersons: incident.linkedPersons || [],
      linkedVehicles: incident.linkedVehicles || [],
    });
    setPersonSearch("");
    setVehicleSearch("");
    setOpen(true);
  ...
  return (
    <div className="h-full flex flex-col gap-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h3 className="text-2xl card-title">{t("tablet.sidebar.incidents")}</h3>
          <p className="card-sub mt-1">{t("tablet.incidents.recent_list")}</p>
        </div>
        <Button onClick={openCreate}>
          <Plus className="w-4 h-4 mr-2" />
          {t("tablet.incidents.new_incident")}
        </Button>
      </div>
        ...
        <div className="space-y-3">
          {ordered.length === 0 ? (
            <div className="min-h-40 flex items-center justify-center text-sm text-[var(--mdt-text-muted)]">
              {t("tablet.incidents.empty")}
            </div>
          ) : (
            ordered.map((incident) => (
              <div key={incident.id} className="rounded-2xl border border-[var(--mdt-border)] bg-[rgba(255,255,255,0.02)] p-4 space-y-3">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-white font-semibold text-lg">{incident.title}</p>
                    <p className="text-xs text-[var(--mdt-text-muted)] mt-1">{incident.location || t("tablet.vehicles.not_available")}</p>
                  </div>
                  <div className="flex items-center gap-2 flex-wrap justify-end">
                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${severityTone[incident.severity]}`}>{incident.severity}</span>
                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${statusTone[incident.status]}`}>{incident.status}</span>
                  </div>
                </div>

                <p className="text-sm text-[var(--mdt-text-muted)] whitespace-pre-wrap break-words">{incident.description}</p>
                ...
                <div className="flex items-center justify-end gap-2 pt-1">
                  <Button variant="ghost" onClick={() => openEdit(incident)}><Pencil className="w-4 h-4 mr-2" />{t("edit")}</Button>
                  <Button variant="ghost" onClick={() => onDelete(incident.id)}><Trash2 className="w-4 h-4 mr-2" />{t("delete")}</Button>
                </div>
              </div>
            ))
          )}
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editingId ? t("edit") : t("tablet.incidents.new_incident") }>
        <div className="space-y-3">
          <div>
